...
static void		free_gen(t_path **gen, int gen_size)
{
	while (--gen_size >= 0)
		free(gen[gen_size]);
}
...
static t_path	*next_gen(t_env *env, t_path *parent, int gen_size)
{
	...
	while (l && l->room && (++i < gen_size))
	{
		...
		if (!l->next)
			break ;
		if (p[i]->room == env->end && (found = duplicate_path(p[i])) && (free_gen(p, gen_size)))
			return (found);
		if ((found = next_gen(env, p[i], count_linked_rooms(p[i]->room))) && (free_gen(p, gen_size)))
			return (found)
		l = l->next;
	}
	return (NULL);
...
void			solve(t_env *env)
{
	...
	env->fastway = next_gen(env, init, count_linked_rooms(init->room));
}
