...
/*   Updated: 2018/06/28 23:21:03 by fmadura          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_check_args(t_asm *comp, int i, int *total_size)
{
	...
	i = ft_check_separator(comp, i, "Bad argument (sti instruction)\n");
	...
	if ((ret = ft_get_type(comp->r_str[i], total_size)) == 1)
		...
		ft_error(comp, "Bad argument (sti instruction)\n");
	if (comp->r_str[i])
		ft_error(comp, "Bad argument (sti instruction)\n");
...
void		ft_sti(t_asm *comp, int i)
{
	...
	ft_get_args(comp, i + ft_tsize(2, (total_size >> 2) & 0x3, 1),
		ft_tsize(2, (total_size >> 2) & 0x3, 0), label_index);
}
