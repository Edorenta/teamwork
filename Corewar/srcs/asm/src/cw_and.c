...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/03 07:02:40 by jyildiz-          #+#    #+#             */
/*   Updated: 2018/07/03 07:04:56 by jyildiz-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cw_check_args(t_asm *comp, int i, int *total_size)
{
	...
	i = cw_check_separator(comp, i, "Bad argument (and instruction)\n");
	if ((ret = cw_get_type(comp->r_str[i], total_size)) == 1)
		i = cw_check_reg(comp, comp->r_str, i);
	else if (ret == 2)
		i = cw_check_dir(comp, comp->r_str, i);
	...
	i = cw_check_reg(comp, comp->r_str, i);
	if (comp->r_str[i])
		cw_error(comp, "Bad argument (and instruction)\n");
