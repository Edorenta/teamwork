...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/06/20 03:25:42 by fmadura           #+#    #+#             */
/*   Updated: 2018/06/29 07:27:37 by fmadura          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "op_asm.h"

static void	ft_update_lbl(t_asm *comp, t_lbl **label, int conf, int pc)
{
	t_idx	*tmp1;
	t_idx	*tmp2;
	...
	else if (!conf)
	{
		...
		if (!(tmp2 = (t_idx*)malloc(sizeof(t_idx) * 1)))
			ft_error(comp, "Malloc error (creating index structure)\n");
		if (!(*label)->replace)
			(*label)->replace = tmp2;
		else
			while (tmp1->next)
...
static void	ft_create_lbl(t_asm *comp, t_lbl **label, int conf, int pc)
{
	if (!(*label = (t_lbl*)malloc(sizeof(t_lbl) * 1)))
		ft_error(comp, "Malloc error (creating label structure)\n");
	...
	else
	{
		...
		if (!((*label)->name = ft_strsub(comp->r_str, 1, conf - 1)))
			ft_error(comp, "Malloc error (creating label)\n");
		(*label)->add = 0;
...
static int	ft_read_unconf_label(t_asm *comp, int start, int pc)
{
	int		ret;
	t_lbl	*tmp;
	char	*name;
	...
	while (ft_strchr(LABEL_CHARS, comp->r_str[start]))
		start++;
	if (!comp->label)
		ft_create_lbl(comp, &comp->label, 0, pc);
	else
	{
		...
		free(name);
		if (ret)
			ft_create_lbl(comp, &tmp->next, 0, pc);
		else
			ft_update_lbl(comp, &tmp, 0, pc);
	}
	return (start);
}

static int	ft_read_conf_label(t_asm *comp, int start, int pc)
{
	int		ret;
	t_lbl	*tmp;
	...
	if (!(comp->r_str = ft_strsub(comp->line, start,
		...
		ft_error(comp, "Malloc error (creating label)\n");
	if (!comp->label)
		ft_create_lbl(comp, &comp->label, 1, pc);
	else
	{
		ret = 1;
		tmp = comp->label;
		while (tmp && (ret = ft_strcmp(tmp->name, comp->r_str)) && tmp->next)
			tmp = tmp->next;
		if (ret)
			ft_create_lbl(comp, &tmp->next, 1, pc);
		else
		{
			ft_update_lbl(comp, &tmp, 1, pc);
			free(comp->r_str);
		}
	}
	comp->r_str = NULL;
...
int			ft_get_label(t_asm *comp, int start, int pc)
{
	...
	if (!start)
	{
		...
		if (comp->line[i] != LABEL_CHAR)
			return (start);
		return (ft_read_conf_label(comp, start, pc));
	}
	return (ft_read_unconf_label(comp, start, pc));
}
